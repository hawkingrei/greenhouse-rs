...
use cibo_util::codec::number::{self, NumberEncoder};
use cibo_util::codec::{Error, Result};
...
mod tests {
    ...
    fn test_get_range_entries_and_versions() {
        ...
        let cfs_opts = LARGE_CFS
            ...
            .collect();
        let db = Arc::new(
            crate::raw_util::new_engine_opt(path_str, db_opts, cfs_opts)
                .unwrap(),
        );
        ...
        let (entries, versions) =
            get_range_entries_and_versions(db.c(), cf, &start_keys, &end_keys)
                .unwrap();
