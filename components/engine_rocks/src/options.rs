...
impl From<engine_traits::ReadOptions> for RocksReadOptions {
    fn from(opts: engine_traits::ReadOptions) -> Self {
        ...
        r.set_verify_checksums(opts.verify_checksums());
        r.set_prefix_same_as_start(opts.prefix_same_as_start());
        r.set_readahead_size(opts.readahead_size());
        RocksReadOptions(r)
    }
}
...
fn build_read_opts(iter_opts: engine_traits::IterOptions) -> RawReadOptions {
    ...
    opts.set_verify_checksums(iter_opts.verify_checksums());
