...
impl Mutable for RocksWriteBatch {
    ...
    fn merge(&mut self, key: &[u8], value: &[u8]) -> Result<()> {
        self.wb.merge(key, value).map_err(Error::Engine)
    }

    fn merge_cf(&mut self, cf: &str, key: &[u8], value: &[u8]) -> Result<()> {
        let handle = get_cf_handle(self.db.as_ref(), cf)?;
        self.wb.merge_cf(handle, key, value).map_err(Error::Engine)
    }
}
