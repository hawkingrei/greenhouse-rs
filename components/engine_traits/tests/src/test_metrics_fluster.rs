...
fn test_metrics_flusher() {
    ...
    let analyzer_path = path.path().join(Path::new("analyzer"));
    ...
    let analyzer_engine = rocks_util::new_engine_opt(
        analyzer_path.to_str().unwrap(),
    ...
    let engines = Engines::new(analyzer_engine, engine, raft_engine);
